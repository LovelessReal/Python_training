...
print(my_second_set) #Отобразятся данные в единственном экземпляре и в своем порядке (обычно, по возрастанию)

#Тип данных словарь. Dictionary. Dict

my_dict = {               #Оформляется как set, но с другим содержимым (так что система понимает)
     'name': 'Bob', 
     'age': '28',
     'nickname': 'Uchiha'
     }
